...
metric_names = [
    "node-is-malicious-accuracy",
    "node-is-malicious-auc",
    "node-is-attacked-accuracy",
    "node-is-attacked-auc",
    "edge-is-malicious-accuracy",
    "edge-is-malicious-auc",
]
...
def read_results(path):
    ...
    for metric_name in metric_names:
        output[metric_name] = np.mean(
            [
                max(epoch_metrics[metric_name] for epoch_metrics in trial_metrics)
...
if __name__ == "__main__":
    ...
    results = pd.DataFrame.from_records(
        list(read_all("multiruns/all-configs/kitsune-mem*"))
    )
    results = results[results.feats == "ports_and_size"]
    results = results.append(
        pd.DataFrame.from_records(
            list(read_all("multiruns/all-configs/kitsune-lang-lstm-1-64-p*"))
        )
    )
    results = results.append(
        pd.DataFrame.from_records(
            list(read_all("multiruns/all-configs/kitsune-lang-fastgrnn-3-64-p*"))
        )
    )
    results = results.append(
        pd.DataFrame.from_records(
            list(read_all("multiruns/all-configs/kitsune-lang-gru-3-64-p*"))
        )
    )
    latex_prepare(results)
