...
def load_state_dict(baseFolder, fromCheckpoint):
    if fromCheckpoint:
        state_dict = torch.load(util.latestCheckpoint(baseFolder), map_location="cpu")[
            "model"
        ]
    else:
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "run_folder", type=str, help="the trained output must be saved in this folder"
    )
    parser.add_argument(
        "--from-checkpoint",
        action="store_true",
        help="use the latest checkpoint file instead of the model.pt file",
    )
