...
class DataLoader:
    ...
    def loadData(self, tasks):
        ...
        with TimedStep('Sorting'):
            data.sort_values(self.timestampColumn, kind = 'quicksort', inplace = True, ignore_index = True)

        with TimedStep("Generating node IDs"):
            id2ip = pd.unique(data[[self.sourceColumn, self.targetColumn]].values.ravel('K'))
            ip2id = {ip: id for id, ip in enumerate(id2ip)}
            data.loc[:,(self.sourceColumn, self.targetColumn)] = data[[self.sourceColumn, self.targetColumn]].map(ip2id.get)
            E = pandas2torch(data[[self.sourceColumn, self.targetColumn]], dtype = np.int64)
        ...
        with TimedStep('Computing edge features'):
            ...
            if not Xnum.empty:
                vMin, med, vMax = map(lambda t: t[1], Xnum.quantile([0,.5,1]).iterrows())
