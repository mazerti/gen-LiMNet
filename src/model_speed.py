...
def measure_speed(baseFolder, conf, samples):
    ...
    validData = torch.utils.data.DataLoader(
        validDataset, batch_size=1, collate_fn=model.makeBatch, num_workers=2
    )
    ...
    try:
        ...
        model.load_state_dict(
            torch.load(util.latestCheckpoint(baseFolder), map_location="cpu")["model"]
        )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "base_folder", type=str, help="the folder containing the trained model"
    )
    parser.add_argument(
        "--samples",
        type=int,
        default=10,
        help="number of samples to run (higher means more precise speed results",
    )
