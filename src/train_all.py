...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "root_folder",
        type=str,
        help="the results for each configuration will be stored in folder multiruns/<root_folder>/<config_name>",
    )
    parser.add_argument(
        "configs",
        type=str,
        nargs="?",
        help="path of the file containing all the configurations to train",
    )
    parser.add_argument(
        "--gpu",
        type=int,
    ...
    with open(f"{rootFolder}/configs.py", "w") as target, open(
        args.configs, "r"
    ) as input:
